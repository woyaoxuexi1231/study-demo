...
public class SpringTransactionService implements ApplicationContextAware {
    ...
    public void handleTransaction() {
        // 1. 手动开启事务
        PlatformTransactionManager pm = (PlatformTransactionManager) applicationContext.getBean("mybatisTransactionManager");
        DefaultTransactionDefinition definition = new DefaultTransactionDefinition();
        TransactionStatus status = pm.getTransaction(definition);
        try {
            JdbcTemplate jdbcTemplate = (JdbcTemplate) applicationContext.getBean("mybatisTransactionManager");
