...
public class IONote {
     ...
     * 同步阻塞 IO - 需要等待内核 IO 操作彻底完成才能继续做别的事情. (从物理设备复制到内核缓冲区, 再从内核缓冲区复制到用户缓冲区)
     * 同步非阻塞 IO - 内核缓冲区没有数据时直接返回调用失败的信息. 内核缓冲区有数据的情况下, 在复制数据的过程中系统调用是阻塞的. (内核有数据时, 用户进程会阻塞的复制数据)
     * IO 多路复用 - 通过引入 select/epoll 系统调用, 一个进程可以监视多个文件描述符, 一旦有某个描述符就绪(内核缓冲区可读/可写), 内核就把就绪状态返回给应用程序
     *              轮询获取 IO 就绪状态的过程用户线程是阻塞的, 复制数据的过程用户线程也是阻塞的.
     * 信号驱动 IO - 用户线程通过向核心注册 IO 事件的回调函数来避免 IO 事件查询的阻塞. 从内核缓冲区复制到用户缓冲区的过程应用进程是阻塞的.
     * 异步 IO - 用户线程通过系统调用向核心注册某个 IO 操作, 内核在整个 IO 操作完成后通知用户程序, 用户执行后续操作. (用户线程全过程非阻塞)
     * */
}
