    ...
    3) 类加载器
        ...
        双亲委派机制
            在 Java 虚拟机的角度上来说, 只存在两种不同的类加载器:
                ...
                另外一种就是其他的所有的类加载器, 这些类加载器由 Java 语言实现, 独立存在于 Java 虚拟机的外部, 并且全部继承于 ClassLoader 类
            在开发人员的角度来说, 分为三层结构:
            ...
            双亲委派模型的工作过程是: 如果一个类加载器收到了类加载的请求, 他不会首先自己去加载这个类, 而是把请求委派给它的父类加载器去完成, 每一个层次的加载器都是如此
                                    因此所有的加载请求最终都会传到最顶层的启动类加载器, 只有当父类加载器反馈自己无法完成对类的加载的时候, 子加载器才会自己去加载
            使用双亲委派机制的一个好处就是, 类似于 jdk rt.jar 包内的这种类, 始终都会由启动类加载器来加载, 进而保证在各种加载器环境中都能够保证加载的类是同一个类
            所以这里如果我们自己去声明一些 jdk 官方存在的一些同名类, 这些类是不会生效的, 也不会被加载
        破坏双亲委派机制
            1. 在 JDK 1.2 之前, 由于双亲委派机制还未提出
            2. SPI的出现使得双亲委派机制不得不被破坏掉, 而在 JDK 6 之后, 提供了 ServiceLoader 才得以合理的解决
            3. 第三次被破坏出现在 "热", 类似代码热替换, 模块热部署(OSGi)-Java模块化
