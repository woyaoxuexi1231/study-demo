package com.hundsun.demo.springboot.mq;
...
public class mq_note {
    ...
    集群:
    当客户端连接到 RabbitMQ 集群中的非主节点（从节点）时，后续的行为通常会经历以下流程：
    1. 客户端连接到从节点：客户端向 RabbitMQ 集群中的从节点发起连接请求。
    2. 从节点接收连接并处理认证：从节点会接收客户端的连接请求，并进行认证处理，确保客户端具有足够的权限来执行后续操作。
    3. 从节点转发请求到主节点：一旦客户端通过认证，从节点会将客户端的请求转发至集群中的主节点，因为从节点不存储队列的实际数据，它需要将请求发送到实际存储数据的主节点上进行处理。
    4. 主节点处理请求并返回响应：主节点接收到来自客户端的请求后，会处理请求并返回相应的响应给从节点。
    5. 从节点将响应返回给客户端：从节点收到主节点的响应后，将响应返回给客户端。由于从节点只是转发请求和响应的中间人，不参与实际的消息处理或存储，因此它仅负责将主节点的响应传递给客户端。
    这样，客户端连接到 RabbitMQ 集群的从节点后，后续的操作都会在集群的主节点上进行处理，而从节点则负责将请求和响应在客户端和主节点之间进行转发，以实现负载均衡和高可用性。

    在 RabbitMQ 集群模式中，消息并不是直接发送到主节点。RabbitMQ 采用的是分布式系统架构，所有节点都可以接收和处理消息。
    当客户端连接到集群中的任何一个节点时，这个节点都可以处理客户端发送的消息。具体发送到哪个节点的队列上取决于 RabbitMQ 集群的配置和路由规则。
    RabbitMQ 的集群模式并没有明确的“主节点”概念，所有节点都参与消息的处理和路由。消息最终将被分发到适当的队列，并且其他节点上也会保持相同的队列结构和消息数据的同步。
    因此，在 RabbitMQ 集群中，客户端发送的消息可能会被任意一个节点接收，并根据路由规则将消息发送到适当的队列中。
     */
