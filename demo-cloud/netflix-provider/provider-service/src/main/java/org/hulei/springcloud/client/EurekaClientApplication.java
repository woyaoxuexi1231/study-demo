...
public class EurekaClientApplication implements ApplicationRunner {
    ...
    public static void main(String[] args) {
        /*
        Eureka 这一套主要的注册逻辑在 EurekaClientAutoConfiguration
        主要参与注册的类是 com.netflix.discovery.DiscoveryClient
        注册的信息如下：
            InstanceInfo [instanceId = 192.168.81.1:eureka-client:10002, appName = EUREKA-CLIENT, hostName = 192.168.3.2, status = UP, ipAddr = 192.168.3.2, port = 10002, securePort = 443, dataCenterInfo = com.netflix.appinfo.MyDataCenterInfo@1d2ec1f

        DiscoveryClient 创建阶段会注册两个定时任务
          heartbeatTask - 心跳任务，定期发送那个心跳请求，表示服务还活着
          instanceInfoReplicator - 服务信息复制器，定期检测本地服务信息是否有变化，如果有变化，就会重新注册或更新到 eureka-server
         */
