...
-- 定义游标  查询一个表的  未使用空间大小/(数据长度+索引长度) > 0.1
  DECLARE cur CURSOR FOR select concat(table_schema,'.',table_name) from information_schema.tables where table_schema in('ast_db') and round(data_free/(data_length+index_length),2)>=0.1;
