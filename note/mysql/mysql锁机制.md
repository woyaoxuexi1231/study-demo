...
在这个示例中，每个版本都与对应的事务关联起来。当一个事务开始时，它会创建一个 Read-View，该 Read-View 包含了事务开始时数据库中已提交的数据的版本。根据隔离级别的不同，Read-View 可能会包含更旧的或更新的版本。例如，Repeatable Read 隔离级别下，T1 和 T2
