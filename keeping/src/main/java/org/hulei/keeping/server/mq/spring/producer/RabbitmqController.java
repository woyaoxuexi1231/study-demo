package org.hulei.keeping.server.mq.spring.producer;
...
public class RabbitmqController {
    ...
    public ResultDTO<?> sentSampleMsg() {
        try {
            ...
            // this.pushDirectMsg(idempotency, messagePostProcessor, correlationData);
    ...
    private void pushDirectMsg(MQIdempotency message, MessagePostProcessor messagePostProcessor, CorrelationData correlationData) {
        // 使用此方法, 交换机会马上把所有的信息都交给所有的消费者, 消费者再自行处理, 不会因为消费者处理慢而阻塞线程。
        rabbitTemplate.convertAndSend(
                "amq.direct", // the exchange.
                "direct.key.test", // the routing key.
                JSON.toJSONString(message), // the data to send.
                messagePostProcessor, // a message post processor (can be null).
                correlationData // correlation data (can be null).
        );
