...
public class Collection {
    ...
    /**
     * 非线程安全
     * 每次扩容到原来的 1.5 倍 (int newCapacity = oldCapacity + (oldCapacity >> 1);), 如果扩容后大小依旧不够, 那么扩容的大小直接变成刚好能够容纳添加后的数组大小
     * 扩容后通过 Arrays.copyOf 获得新的数组, 底层使用 java.lang.System#arraycopy实现, 这个方法直接
    ...
    /**
     * 非线程安全
     * 查找和新增都是通过元素的 hash 值来计算下标, 可以插入 null, 对于 null, hash值默认给 0
