...
public class RibbonController {
    ...
    @HystrixCommand 注解是 Netflix 的 Hystrix 框架提供的一种方式，用于对方法进行声明式的熔断、降级、隔离等操作，以增强微服务架构的可靠性和稳定性。下面是该注解的详细解释：
      ...
      groupKey：指定命令所属的组，用于对命令进行分组。默认值是被注解方法的运行时类名。
      commandKey：指定命令的键名，用于在监控、度量、报警等方面进行识别和区分。默认值是被注解方法的名称。
      threadPoolKey：指定线程池的键名，用于指定命令执行的线程池。默认情况下，命令使用 Hystrix 默认的线程池。Hystrix默认以类为单位建立线程池
      fallbackMethod：指定命令执行失败时调用的回退方法名。回退方法应该在与被注解方法相同的类中，并且方法签名应该与被注解方法相同。

      commandProperties：指定命令的属性，用于配置 Hystrix 命令的行为，比如设置命令的超时时间、执行是否允许短路、执行超时的等待时间等。
      threadPoolProperties：指定线程池的属性，用于配置 Hystrix 线程池的行为，比如设置线程池大小、队列大小、拒绝策略等。
      ignoreExceptions：指定忽略的异常列表，这些异常在触发时会被忽略，不会触发熔断机制。
      observableExecutionMode：指定命令的执行模式，可以是 EAGER（即时执行）或 LAZY（延迟执行）。默认是 EAGER，即立即执行命令。
      raiseHystrixExceptions：指定需要包装的异常列表，这些异常会被包装在 HystrixRuntimeException 中，用于在调用者层级处理异常。
      defaultFallback：指定默认的回退方法名，用于处理所有命令的失败情况。这个方法不能有参数，返回类型应与命令的返回类型兼容。
