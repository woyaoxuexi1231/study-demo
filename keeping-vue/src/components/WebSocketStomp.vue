...
<script>
/*
* `webstomp-client` 和 `stompjs` 都是在 JavaScript 环境下实现 STOMP 协议（Simple (or Streaming) Text Oriented Message Protocol）的客户端库。
* 尽管它们的目标相同，即在 Web 应用中通过 WebSocket 提供 STOMP 协议支持，但它们之间存在一些区别：
...
export default {
  ...
  methods: {
    ...
    connectWebSocket() {
      ...
      const socket = new SockJS('/api/gs-guide-websocket'); // 连接到 WebSocket 服务器端点
      // const socket = new SockJS('http://localhost:10001/gs-guide-websocket'); // 连接到 WebSocket 服务器端点
