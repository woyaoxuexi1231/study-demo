...
local function create_new(id_number, count, conf, ctx)
    ...
    core.log.warn("custom-limit-plugin initialing: id_number: ", id_number, " count: ", count, " key: ", key)
    ...
    core.log.warn("custom-limit-plugin successful: id_number: ", id_number, " count: ", count, " key: ", key)

    return setmetatable(self, mt)
end

local function gen_limit_obj(id_number, count, conf, ctx)
    ...
    return core.lrucache.plugin_ctx(lrucache, ctx, plugin_name, create_new, id_number, count, conf, ctx)
end
...
function _M.access(conf, ctx)
    ...
    if status == 200 then
        ...
        if (is_admin == false) then
            ...
            local lim, err = gen_limit_obj(id_number, visited_freq, conf, ctx)
            if not lim then
