        ...
        return builder.routes()
                .route( // 创建新路由,对于请求路径为 “/get” 的请求，只有第一个路由规则会被匹配到并执行，即使有第二个路由规则也匹配路径为 “/get”。因此，第二个路由规则将被忽略，不会被执行。
                        r -> r.path("/get") // 表示这个路由规则会匹配请求路径为 “/get” 的请求
                                .filters(f -> f.filter(new RequestTimeFilter()).addRequestHeader("X-Response-Default-Foo", "Default-Bar")) // 在转发请求的响应中添加了一个名为 X-Response-Default-Foo 值为 Default-Bar 的头信息
                                .uri("http://httpbin.org:80") // 设置路由的 URI。
                )
                // // .route( // 创建新路由
