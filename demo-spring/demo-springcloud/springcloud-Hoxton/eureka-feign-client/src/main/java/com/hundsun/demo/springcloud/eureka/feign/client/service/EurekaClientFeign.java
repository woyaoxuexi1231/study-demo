 ...
 * Feign 客户端的主要作用包括：
 * <p>
 * 1. **简化服务调用**：通过 Feign 客户端，你可以用接口的方式来声明服务间的调用，而不需要手动编写 HTTP 请求和处理响应。这样大大简化了服务调用的流程和代码编写。
 * <p>
...
/*
`@FeignClient` 注解是 Spring Cloud 中用于声明 Feign 客户端的注解。它用于创建一个 Feign 客户端，并指定要调用的服务名称、自定义的配置、以及断路器（Hystrix）的回退实现。

具体来说，`@FeignClient` 注解的各个属性含义如下：

- `value`：指定要调用的服务名称。这个服务名称对应于 Eureka 注册中心中注册的服务名，Feign 客户端将根据这个服务名来查找服务实例并进行调用。

- `configuration`：指定自定义的 Feign 配置类。你可以在这个配置类中配置 Feign 客户端的相关参数，比如超时时间、重试策略等。

- `fallback`：指定断路器（Hystrix）的回退实现类。如果调用服务失败或者出现异常，断路器会启用，并调用指定的回退实现类来处理回退逻辑。

综合来看，`@FeignClient` 注解用于声明一个 Feign 客户端，并指定要调用的服务名称、自定义的配置以及断路器的回退实现。通过这个注解，我们可以方便地创建和配置 Feign 客户端，并实现服务之间的远程调用。
 */
...
public interface EurekaClientFeign {
    ...
    @GetMapping("/hi2")
    String sayHi2FromClientEureka(@RequestParam(required = false, name = "req") String req, @RequestParam(required = false, name = "req2") String req2);
}
